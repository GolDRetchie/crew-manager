...
/* ====================================================================
   Crew Manager — mp-crew.js  (STAP 4c)

   Online crew & line-up, opnieuw opgezet volgens de goedgekeurde mockup:
   - Generieke dek-slots (rol reist met het karakter; plek telt niet).
     Dek = min(cap, 9), bank = max(2, cap-9), altijd minstens 2 bankplekken.
   - Eigen, customisable schip (CSS-vars): top-down op het crew-scherm,
     side-profile in de bewerk-editor.
   - Kwast-knop -> bewerk-scherm met kwasten op het schip (Bow/Hull/Deck/
     Trim/Sail/Flag), per-zone drawer, lopend totaal -> Finish -> samenvatting
     -> Finish & pay (Api.saveCosmetics). Side/Top-view toggle boven het schip.
   - Upgrade-knop met bevestiging (prijs + Shipwright-korting) via openModal.

   Rendert in #cp-content. window.cmOpenCrew(worldId).
   Verbruikt: Api.getLineup / saveLineup / upgradeShip / saveCosmetics,
   en (uit game.js) openModal, colorFor, initial, escapeHtml, fmtShort.
   Single-player (game-crew.js) blijft ongemoeid.
   ==================================================================== */
...
(function () {
  ...
  /* ---------- tier-info (moet matchen met server/src/config/shipTiers.ts) ---------- */
  var NEXT_TIER = {
  ...
  /* generieke slot-posities per dek-aantal (alleen 3/7/9 komen voor) */
  var SLOTPOS = {
    3:[{l:50,t:43},{l:33,t:64},{l:67,t:64}],
    7:[{l:35,t:30},{l:65,t:30},{l:30,t:48},{l:70,t:48},{l:35,t:66},{l:65,t:66},{l:50,t:81}],
    9:[{l:35,t:24},{l:65,t:24},{l:28,t:39},{l:72,t:39},{l:35,t:54},{l:65,t:54},{l:28,t:69},{l:72,t:69},{l:50,t:83}],
  };
  var CAPTAIN_POS = { l:50, t:9 };

  /* kwasten per view */
  var SIDE_BRUSHES = [
    {zone:"figurehead",l:10,t:73,label:"Bow"},
    {zone:"hullColor", l:46,t:67,label:"Hull"},
    {zone:"deckColor", l:70,t:53,label:"Deck"},
    {zone:"trimColor", l:26,t:55,label:"Trim"},
    {zone:"sailColor", l:42,t:24,label:"Sail"},
    {zone:"jollyRoger",l:50,t:37,label:"Flag"},
  ];
  var TOP_BRUSHES = [
    {zone:"hullColor", l:14,t:42,label:"Hull"},
    {zone:"deckColor", l:50,t:64,label:"Deck"},
    {zone:"trimColor", l:86,t:42,label:"Trim"},
    {zone:"jollyRoger",l:50,t:50,label:"Flag"},
  ];

  var C = { id:null, data:null, byName:{}, lineup:null, ship:null, mode:"portrait",
            edView:"side", edit:null, original:null };

  /* ====================================================================
     SVG — Jolly Rogers (origineel: skull + opvallend element)
     ==================================================================== */
  ...
  /* ====================================================================
     SVG — boegbeelden (zijaanzicht, naar links, gecentreerd op 0,0)
     ==================================================================== */
  var FH = {
  ...
  function fhSideAll(){
    return FH_LIST.map(function (n){
      if (n === "none") return '<g class="cs-fh cs-fh-none"></g>';
      return '<g class="cs-fh cs-fh-' + n + '"><g transform="translate(54 196)">' + FH[n] + '</g></g>';
    }).join("");
  }

  /* ====================================================================
     SVG — top-down schip (crew-scherm)
     ==================================================================== */
  function topdownContent(g){
    return '' +
      '<path d="M150 26 C210 70,268 150,270 300 C271 430,235 520,150 572 C65 520,29 430,30 300 C32 150,90 70,150 26 Z" fill="var(--cs-hull)" stroke="var(--cs-trim)" stroke-width="9"/>' +
      ...
      '<g fill="var(--cs-trim)"><circle cx="92" cy="150" r="5"/><circle cx="208" cy="150" r="5"/><circle cx="74" cy="260" r="5"/><circle cx="226" cy="260" r="5"/><circle cx="74" cy="360" r="5"/><circle cx="226" cy="360" r="5"/><circle cx="96" cy="460" r="5"/><circle cx="204" cy="460" r="5"/></g>' +
      '<g fill="rgba(0,0,0,.22)"><rect x="40" y="270" width="16" height="9" rx="2"/><rect x="244" y="270" width="16" height="9" rx="2"/><rect x="44" y="340" width="16" height="9" rx="2"/><rect x="240" y="340" width="16" height="9" rx="2"/></g>' +
      '<line x1="150" y1="26" x2="150" y2="8" stroke="var(--cs-trim)" stroke-width="5" stroke-linecap="round"/>' +
  ...
  function topdownSVG(){
    if (C.mode === "landscape")
      return '<svg class="cs-svg" viewBox="0 0 600 300" preserveAspectRatio="xMidYMid meet"><g transform="translate(0 300) rotate(-90)">' + topdownContent(90) + '</g></svg>';
    return '<svg class="cs-svg" viewBox="0 0 300 600" preserveAspectRatio="xMidYMid meet">' + topdownContent(0) + '</svg>';
  }
  function remap(p){ return (C.mode === "landscape") ? { l:p.t, t:100 - p.l } : p; }

  /* ====================================================================
     SVG — side-profile schip (editor)
     ==================================================================== */
  function shipSideContent(){
    return '' +
      '<line x1="320" y1="50" x2="120" y2="190" stroke="rgba(0,0,0,.22)" stroke-width="2"/>' +
      '<line x1="320" y1="50" x2="545" y2="190" stroke="rgba(0,0,0,.22)" stroke-width="2"/>' +
      '<line x1="320" y1="50" x2="60" y2="194" stroke="rgba(0,0,0,.2)" stroke-width="2"/>' +
      ...
      '<g stroke="rgba(0,0,0,.06)" stroke-width="2"><line x1="270" y1="84" x2="270" y2="192"/><line x1="322" y1="86" x2="322" y2="194"/><line x1="374" y1="84" x2="374" y2="192"/></g>' +
      ...
      '<g stroke="rgba(0,0,0,.13)" stroke-width="1.6" fill="none"><path d="M84 222 C150 250,470 250,560 216"/><path d="M98 238 C160 264,450 264,548 232"/></g>' +
      '<path d="M70 198 L120 188 L548 188 C566 188,576 196,572 210 L70 210 Z" fill="var(--cs-trim)"/>' +
      '<g fill="var(--cs-deck)"><rect x="150" y="178" width="4" height="11"/><rect x="220" y="178" width="4" height="11"/><rect x="290" y="178" width="4" height="11"/><rect x="360" y="178" width="4" height="11"/><rect x="430" y="178" width="4" height="11"/><rect x="500" y="178" width="4" height="11"/></g>' +
      '<line x1="150" y1="180" x2="504" y2="180" stroke="var(--cs-deck)" stroke-width="3"/>' +
      ...
      '<g stroke="var(--cs-trim)" stroke-width="2" fill="none"><circle cx="200" cy="234" r="8"/><circle cx="280" cy="242" r="8"/><circle cx="360" cy="244" r="8"/><circle cx="440" cy="238" r="8"/></g>' +
      '<path d="M560 250 L575 256 L569 282 L556 271 Z" fill="var(--cs-trim)"/>' +
      '<path d="M500 188 L500 160 C500 156,504 154,508 154 L540 154 C544 154,548 158,548 162 L548 188 Z" fill="var(--cs-hull)" stroke="var(--cs-trim)" stroke-width="3"/>' +
      '<rect x="512" y="164" width="14" height="14" rx="2" fill="rgba(0,0,0,.28)"/>' +
      fhSideAll();
  }

  /* ====================================================================
     Cosmetics toepassen (CSS-vars + data-attributen)
     ==================================================================== */
  ...
  window.cmOpenCrew = function (worldId){
    ...
    injectCss();
    content().innerHTML = '<div class="cs-loading">Loading\u2026</div>';
    load();
  };

  function orientation(){
    try { return window.matchMedia("(orientation: landscape)").matches ? "landscape" : "portrait"; }
  ...
  function deckSlotHtml(i){
    var p = remap(SLOTPOS[C.lineup.deck.length] ? SLOTPOS[C.lineup.deck.length][i] : { l:50, t:50 });
    var style = "left:" + p.l + "%; top:" + p.t + "%";
    var name = C.lineup.deck[i];
    if (name){
      var m = C.byName[name] || {};
      var tired = (m.cond != null && m.cond < 80) ? '<span class="cs-tired" title="Tired (< 80)">\u26A1</span>' : "";
      return '<div class="cs-slot filled" style="' + style + '" data-drop="deck:' + i + '" data-drag="deck:' + i + '">' +
        tired +
        '<div class="cs-nm"><span class="cs-dot" style="background:' + col(name) + '"></span>' + esc(name) + '</div>' +
        '<div class="cs-ro">' + esc(m.role || "") + '</div></div>';
    }
    return '<div class="cs-slot empty" style="' + style + '" data-drop="deck:' + i + '"><div class="cs-nm">+ Empty</div></div>';
  }
  function benchSlotHtml(i){
    var name = C.lineup.bench[i];
    if (name){
      var m = C.byName[name] || {};
      return '<div class="cs-bslot filled" data-drop="bench:' + i + '" data-drag="bench:' + i + '">' +
        '<span class="cs-bav" style="background:' + col(name) + '">' + ini(name) + '</span>' +
        '<span class="cs-binfo"><b>' + esc(name) + '</b><i>' + esc(m.role || "") + ' \u00b7 rests to full</i></span></div>';
    }
    return '<div class="cs-bslot empty" data-drop="bench:' + i + '">Empty bench slot</div>';
  }

  function shipStripHtml(){
    var ship = C.ship || { shipTier: 1, tierName: "Dinghy" };
    var atMax = (ship.shipTier >= 3);
    var upg = atMax ? '<span class="cs-ship-max">Max tier</span>'
                    : '<button class="cs-ship-upg" id="cs-upg" type="button">\u2B06 Upgrade</button>';
    return '<div class="cs-ship">' +
        '<span class="cs-ship-pip">\u26F5</span>' +
        '<span class="cs-ship-meta"><b>Ship &middot; Tier ' + ship.shipTier + '</b><i>' + esc(ship.tierName || "") + '</i></span>' +
        '<span class="cs-ship-sp"></span>' + upg +
        '<button class="cs-ship-brush" id="cs-brush" type="button" title="Customize ship" aria-label="Customize ship">\uD83D\uDD8C</button>' +
      '</div>';
  }
  ...
  function render(){
    var d = C.data;
    var roster = d.squad || [];
    var cap = d.rosterCap || 13;
    var deckN = C.lineup.deck.length, benchN = C.lineup.bench.length;

    var deckHtml = "";
    for (var i = 0; i < deckN; i++) deckHtml += deckSlotHtml(i);
    var capPos = remap(CAPTAIN_POS);
    var capHtml = '<div class="cs-slot cap filled" style="left:' + capPos.l + '%; top:' + capPos.t + '%">' +
        '<div class="cs-nm">' + esc(d.captain) + '</div><div class="cs-ro">Captain</div></div>';

    var benchHtml = "";
    for (var b = 0; b < benchN; b++) benchHtml += benchSlotHtml(b);

    content().innerHTML =
      '<div class="cs-root" data-mode="' + C.mode + '">' +
        '<div class="cw-top">' +
          ...
          '<div class="cw-bal">' +
            '<div class="mini-stat"><span class="mini-stat__label">Bounty</span><span class="mini-stat__val">' + short(totalBounty()) + '</span></div>' +
            '<div class="mini-stat"><span class="mini-stat__label">Crew</span><span class="mini-stat__val">' + (roster.length + 1) + ' / ' + (cap + 1) + '</span></div>' +
          '</div>' +
        ...
        shipStripHtml() +
        '<div class="cs-main">' +
          '<div class="cs-stage-wrap"><div class="cs-stage">' + topdownSVG() + capHtml + deckHtml + '</div>' +
            '<div class="cs-hint">' + (deckN < 9 ? (deckN + ' deck posts \u2014 any role fits any spot.') : 'Only your 9 on deck fight.') + '</div></div>' +
          '<div class="cs-bench-col"><div class="cs-bench"><div class="cs-bench-h">Bench <span>rest \u2192 full</span></div>' + benchHtml + '</div></div>' +
        '</div>' +
      '</div>';
  ...
  function onDragStart(e){
    ...
    g.innerHTML = '<span class="cs-dot" style="background:' + col(name) + '"></span>' + esc(name);
  ...
  function applyDrop(tType, tKey){
    ...
    setPlace(drag.type, drag.key, occupant);   // swap of verplaats
    saveSoon();
  }
  function endDrag(apply, x, y){
    ...
    if (apply && drag){
      ...
      if (t){ var parts = t.dataset.drop.split(":"); applyDrop(parts[0], parseInt(parts[1], 10)); }
    }
  ...
  /* ====================================================================
     Upgrade (bevestiging met prijs + Shipwright-korting)
     ==================================================================== */
  ...
  /* ====================================================================
     Ship editor (side / top, kwasten, drawer, finish -> samenvatting)
     ==================================================================== */
  function ensureEditor(){
    if (el("cs-editor")) return;
    var ov = document.createElement("div");
    ov.className = "cs-editor"; ov.id = "cs-editor";
    ov.innerHTML =
      '<div class="cs-ed-head">' +
        '<button class="btn-ghost" id="cs-ed-cancel" type="button">\u2715 Cancel</button>' +
        '<h2>Customize ship</h2>' +
        '<div class="cs-ed-total"><span>Total</span><b id="cs-ed-total">\u0E3F 0</b></div>' +
        '<button class="cs-ed-finish" id="cs-ed-finish" type="button">Finish</button>' +
      '</div>' +
      '<div class="cs-ed-body">' +
        '<div class="cs-view-toggle"><button class="btn-ghost" id="cs-view" type="button">\u21C5 Top view</button></div>' +
        '<div class="cs-ed-stage" id="cs-ed-stage"></div>' +
        '<div class="cs-ed-scrim" id="cs-ed-scrim"></div>' +
        '<div class="cs-drawer" id="cs-drawer"><h4><span id="cs-drawer-ttl">Hull color</span><span class="cs-x" id="cs-drawer-x">\u2715</span></h4>' +
          '<div class="cs-opts" id="cs-opts"></div><div class="cs-applied">Live preview \u2014 changes show on the ship.</div></div>' +
        '<div class="cs-summary" id="cs-summary">' +
          '<div class="cs-ed-head"><button class="btn-ghost" id="cs-sum-back" type="button">\u2039 Back</button><h2>Review changes</h2></div>' +
          ...
          '<div class="cs-acts"><button class="btn-ghost" id="cs-sum-back2" type="button">Back</button><button class="cs-ed-finish" id="cs-pay" type="button">Finish &amp; pay</button></div>' +
        '</div>' +
      '</div>';
  ...
  function openEditor(){
    ensureEditor();
    C.original = packShip(C.ship);
    C.edit = packShip(C.ship);
    C.edView = "side";
    el("cs-view").textContent = "\u21C5 Top view";
    el("cs-summary").classList.remove("open");
    closeDrawer();
    renderEditorShip();
    refreshTotal();
  ...
  function renderEditorShip(){
    var stage = el("cs-ed-stage"); var brushes, svg;
    if (C.edView === "side"){
      stage.className = "cs-ed-stage side";
      svg = '<svg class="cs-svg" viewBox="0 0 640 360" preserveAspectRatio="xMidYMid meet">' + shipSideContent() + '</svg>';
      brushes = SIDE_BRUSHES;
    } else {
      stage.className = "cs-ed-stage top";
      svg = '<svg class="cs-svg" viewBox="0 0 300 600" preserveAspectRatio="xMidYMid meet">' + topdownContent(0) + '</svg>';
      brushes = TOP_BRUSHES;
    }
    stage.innerHTML = svg + brushes.map(function (b){
      return '<button class="cs-brush" data-zone="' + b.zone + '" style="left:' + b.l + '%;top:' + b.t + '%;" aria-label="Edit ' + b.label + '">\uD83D\uDD8C<span class="cs-blbl">' + b.label + '</span></button>';
    }).join("");
  }
  ...
  function openDrawer(zone){
    ...
    if (Z.type === "color"){
      Z.palette.forEach(function (c){
        var b = document.createElement("button");
        b.className = "cs-opt color" + (C.edit[Z.field] === c ? " sel" : "");
        b.style.background = c;
        b.onclick = function (){ C.edit[Z.field] = c; applyShip(C.edit); mark(wrap, b); refreshTotal(); };
        wrap.appendChild(b);
      });
    } else if (Z.type === "fh"){
      FH_LIST.forEach(function (name){
        var b = document.createElement("button");
        b.className = "cs-opt preset" + (C.edit.figurehead === name ? " sel" : "");
        b.innerHTML = fhPreview(name);
        b.onclick = function (){ C.edit.figurehead = name; applyShip(C.edit); mark(wrap, b); refreshTotal(); };
        wrap.appendChild(b);
      });
    } else if (Z.type === "jr"){
      JR_LIST.forEach(function (key){
        var b = document.createElement("button");
        b.className = "cs-opt preset" + (C.edit.jollyRoger === key ? " sel" : "");
        b.innerHTML = jrPreview(key);
        b.onclick = function (){ C.edit.jollyRoger = key; applyShip(C.edit); mark(wrap, b); refreshTotal(); };
        wrap.appendChild(b);
      });
    }
    el("cs-ed-stage").querySelectorAll(".cs-brush").forEach(function (x){ x.classList.toggle("active", x.dataset.zone === zone); });
    el("cs-ed-scrim").classList.add("show");
    el("cs-drawer").classList.add("show");
  }
  ...
  function getChanges(){
    ...
    Object.keys(COSMETIC_PRICE).forEach(function (field){
      if (C.edit[field] !== C.original[field]){
        ...
        out.push({
          field: field, label: ZONES[field].label, price: COSMETIC_PRICE[field],
          swatch: isColor ? C.edit[field] : null,
          detail: isColor ? C.edit[field] : (C.original[field] + " \u2192 " + C.edit[field]),
        });
      }
    });
    return out;
  ...
  function refreshTotal(){
    var t = getChanges().reduce(function (s, c){ return s + c.price; }, 0);
    el("cs-ed-total").textContent = berries(t);
  }
  function showSummary(){
    var ch = getChanges(), list = el("cs-sum-list"); list.innerHTML = "";
    if (!ch.length){
      list.innerHTML = '<div class="cs-sum-empty">No changes yet \u2014 tap a \uD83D\uDD8C on the ship to customize.</div>';
      el("cs-sum-trow").style.display = "none";
    } else {
      el("cs-sum-trow").style.display = "";
      ch.forEach(function (c){
        var li = document.createElement("li");
        var icon = c.swatch ? '<span class="cs-chip" style="background:' + c.swatch + '"></span>' : "\uD83D\uDD8C";
        li.innerHTML = '<div class="cs-sum-ic">' + icon + '</div><div class="cs-sum-tx"><div class="t">' + c.label + '</div><div class="d">' + esc(c.detail) + '</div></div><div class="cs-sum-pr">' + berries(c.price) + '</div>';
        list.appendChild(li);
      });
    }
  ...
  async function payAndClose(){
    var ch = getChanges();
    if (!ch.length){ closeEditorOverlay(); return; }
    var changes = {}; ch.forEach(function (c){ changes[c.field] = C.edit[c.field]; });
    var pay = el("cs-pay"); if (pay) pay.disabled = true;
    try {
      var r = await Api.saveCosmetics(C.id, changes);
      C.ship = r.ship || C.ship;
      applyShip(C.ship);
      closeEditorOverlay();
      toast("Ship updated \u2713");
      render();
    } catch (e){
      toast(e.message || "Couldn\u2019t save look");
      if (pay) pay.disabled = false;
    }
  }
  function cancelEditor(){
    C.edit = packShip(C.original);
    applyShip(C.original);   // herstel naar opgeslagen look
    closeEditorOverlay();
  }
  function closeEditorOverlay(){
    var ov = el("cs-editor"); if (ov) ov.classList.remove("open");
    var sm = el("cs-summary"); if (sm) sm.classList.remove("open");
    closeDrawer();
  }

  /* ====================================================================
     Oriëntatie: hele boot kantelt mee
     ==================================================================== */
  try {
  ...
  function injectCss(){
    ...
    css.textContent = [
      ...
      ".cs-loading{padding:40px;text-align:center;color:#aa9c80;font-style:italic;}",
      ...
      /* schip-strip */
      ".cs-ship{display:flex;align-items:center;gap:10px;margin:10px 0 4px;padding:9px 12px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.12);border-radius:11px;}",
      ".cs-ship-pip{width:30px;height:30px;border-radius:8px;display:grid;place-items:center;font-size:16px;background:#1b4332;border:1px solid rgba(255,255,255,.2);}",
      ".cs-ship-meta{display:flex;flex-direction:column;line-height:1.15;}",
      ".cs-ship-meta b{font-size:14px;}.cs-ship-meta i{font-style:normal;font-size:12px;opacity:.7;}",
      ".cs-ship-sp{flex:1 1 auto;}",
      ".cs-ship-upg{cursor:pointer;font:inherit;font-weight:700;color:#2a1c05;background:#e9c46a;border:0;border-radius:10px;padding:8px 14px;}",
      ".cs-ship-upg:hover{filter:brightness(1.06);}.cs-ship-upg:disabled{opacity:.6;}",
      ".cs-ship-max{font-size:12px;opacity:.65;font-style:italic;}",
      ".cs-ship-brush{width:38px;height:38px;border-radius:10px;border:1.5px solid rgba(255,255,255,.22);background:rgba(255,255,255,.04);color:#f3e9d6;font-size:17px;cursor:pointer;}",
      ".cs-ship-brush:hover{background:rgba(255,255,255,.1);}",
      /* layout */
      ".cs-main{display:flex;flex-direction:column;gap:12px;margin-top:10px;}",
      '.cs-root[data-mode="landscape"] .cs-main{flex-direction:row;align-items:stretch;}',
      ".cs-stage-wrap{flex:1.3;display:flex;flex-direction:column;align-items:center;}",
      ".cs-stage{position:relative;margin:0 auto;width:auto;max-width:330px;aspect-ratio:300/560;}",
      '.cs-root[data-mode="landscape"] .cs-stage{max-width:520px;width:100%;aspect-ratio:600/300;}',
      ".cs-svg{position:absolute;inset:0;width:100%;height:100%;overflow:visible;}",
      ".cs-slot{position:absolute;transform:translate(-50%,-50%);width:96px;max-width:30%;border-radius:10px;padding:7px 8px;text-align:center;border:1.5px dashed rgba(255,255,255,.22);background:rgba(8,24,32,.36);}",
      ".cs-slot.filled{border-style:solid;background:linear-gradient(180deg,rgba(243,233,214,.97),rgba(228,214,182,.95));color:#23170a;box-shadow:0 4px 13px rgba(0,0,0,.3);}",
      ".cs-slot.cap{background:#2d6a4f;border:2px solid #f3e9d6;color:#fff;box-shadow:0 6px 16px rgba(0,0,0,.4);}",
      ".cs-slot .cs-nm{font-weight:700;font-style:italic;font-size:12.5px;line-height:1.05;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}",
      ".cs-slot .cs-ro{font-size:9px;letter-spacing:.08em;text-transform:uppercase;margin-top:1px;color:#6b4f29;}",
      ".cs-slot.cap .cs-ro{color:#e9c46a;}",
      ".cs-slot.empty .cs-nm{color:rgba(255,255,255,.5);font-weight:500;font-style:normal;font-size:11.5px;}",
      ".cs-dot{width:7px;height:7px;border-radius:50%;display:inline-block;margin-right:4px;vertical-align:1px;}",
      ".cs-tired{position:absolute;top:-8px;left:-8px;width:20px;height:20px;border-radius:50%;background:#b34747;color:#fff;font-size:11px;display:grid;place-items:center;border:2px solid #0e2a36;}",
      ".cs-slot.droppable{outline:1px dashed rgba(233,196,106,.5);}",
      ".cs-slot.drop-hover{outline:2px solid #e9c46a;}",
      ".cs-slot.is-source{opacity:.4;}",
      ".cs-hint{margin-top:8px;font-size:11px;font-style:italic;color:#aa9c80;text-align:center;}",
      /* bench */
      ".cs-bench-col{flex:.8;}",
      ".cs-bench{background:#123140;border:1px solid rgba(255,255,255,.12);border-radius:14px;padding:11px;}",
      ".cs-bench-h{font-weight:700;font-style:italic;letter-spacing:.05em;text-transform:uppercase;font-size:13px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:center;}",
      ".cs-bench-h span{font-size:10px;color:#aa9c80;font-weight:500;font-style:normal;letter-spacing:.04em;}",
      ".cs-bslot{border:1.5px dashed rgba(255,255,255,.22);border-radius:9px;padding:9px 11px;margin-bottom:7px;font-size:12px;color:#aa9c80;font-style:italic;background:rgba(255,255,255,.02);display:flex;align-items:center;gap:9px;}",
      ".cs-bslot.filled{border-style:solid;font-style:normal;color:#f3e9d6;background:rgba(255,255,255,.05);}",
      ".cs-bslot.drop-hover{outline:2px solid #e9c46a;}.cs-bslot.is-source{opacity:.4;}",
      ".cs-bav{width:26px;height:26px;border-radius:7px;display:grid;place-items:center;color:#fff;font-weight:700;font-size:12px;flex:0 0 auto;}",
      ".cs-binfo{display:flex;flex-direction:column;line-height:1.2;}.cs-binfo b{font-size:12.5px;}.cs-binfo i{font-style:normal;font-size:10px;color:#aa9c80;}",
      ".cs-ghost{position:fixed;z-index:9999;transform:translate(-50%,-50%);pointer-events:none;background:#f3e9d6;color:#23170a;border-radius:9px;padding:6px 10px;font-weight:700;font-style:italic;font-size:12.5px;box-shadow:0 8px 20px rgba(0,0,0,.4);}",
      /* editor overlay */
      ...
      ".cs-ed-finish{cursor:pointer;font:inherit;font-weight:700;color:#2a1c05;background:#e9c46a;border:0;border-radius:10px;padding:8px 14px;}",
      ".cs-ed-finish:disabled{opacity:.6;}",
      ...
      ".cs-view-toggle .btn-ghost{padding:6px 13px;font-size:12px;background:rgba(0,0,0,.3);border-radius:9px;}",
      ...
      ".cs-sum-tx{flex:1;}.cs-sum-tx .t{font-weight:600;font-size:13.5px;}.cs-sum-tx .d{font-size:11px;color:#aa9c80;}",
      ...
      ".cs-sum-total{display:flex;justify-content:space-between;align-items:center;padding:15px 4px 2px;font-size:14px;}.cs-sum-total b{font-weight:700;color:#e9c46a;font-size:22px;}",
      ".cs-acts{display:flex;gap:10px;padding:14px 16px;border-top:1px solid rgba(255,255,255,.12);}.cs-acts>button{flex:1;}",
      /* editor landscape: drawer als zijpaneel */
